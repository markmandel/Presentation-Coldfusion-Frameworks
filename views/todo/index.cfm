...
<uform:form action="#buildURL('todo.add')#"
	id="todo-add"
...
<uform:form action="#buildURL('todo.delete')#"
	id="todo-delete"
	...
	<uform:fieldset legend="Current To-Do's">
		...
		<cfloop array="#rc.list#" index="local.todo">
			<cfscript>
				local.label = "<strong>#todo.getTitle()#</strong>";
				local.class = "";
				if(!isNull(local.todo.getCompletionDate()))
				{
					local.label &= ", Due: <strong>#DateFormat(todo.getCompletionDate(), 'medium')#</strong>";

					if(local.todo.isOverdue())
					{
						local.class = "overdue";
					}
				}

				if(!isNull(local.todo.getImportance()))
				{
					local.label &= " ( Importance: #todo.getImportance()# )";
				}
            </cfscript>
